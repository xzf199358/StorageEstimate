...
def CalStorage(hits, HitsThreshold, med_cubing, best_cubing,cubing4, storage):
    ...
    for i in range(0, len(hits)):
        if hits[i] > HitsThreshold or med_cubing[i] == 0:
            ...
            ###########2层混放############
            if storage[i] >1 and cubing4[i] == 0:
                if storage[i] >= med_cubing[i]:
                    ...
                    MultipSku_pallet_no += 1/2
            else :
                #4层
                MultipSku_pallet_no += 1/4

    print("纯放SKU需要的pallet数量：%d" %SingleSku_pallet_no)
    print("混放SKU需要的pallet数量：%d" %(int(MultipSku_pallet_no)))
    print("总需pallet数量：%d" %(SingleSku_pallet_no +int(MultipSku_pallet_no)))
...
if __name__ == "__main__":
    ...
    try:
        ...
        #######频次###########
        sql = "select hits from layer3_sku.basic_iteam where  (storage_type = 1 OR storage_type = 2) and storagee <> 1 order by hits desc "
        ...
        print(10)
        ####################库存######################
        sql = "select storagee from layer3_sku.basic_iteam where (storage_type = 1 OR storage_type = 2) and storagee <> 1 order by hits desc"
        database_cursor.execute(sql)
        ...
        # print(storage)
        #######################半箱###############
        sql = "select med_cubing from layer3_sku.basic_iteam where (storage_type = 1 OR storage_type = 2) and storagee <> 1  order by hits desc"
        database_cursor.execute(sql)
        ...
        #######################整箱############
        sql = "select best_cubing from layer3_sku.basic_iteam where (storage_type = 1 OR storage_type = 2) and storagee <> 1 order by hits desc"
        database_cursor.execute(sql)
        ...
        ############# #item信息#############
        Itemid = []
        sql = "select id from layer3_sku.basic_iteam where (storage_type = 1 OR storage_type = 2) and storagee <> 1 order by hits desc"
        database_cursor.execute(sql)
        ...
        for i in range(0,len(percentage)):
            ...
            CalStorage(hits, HitsThreshold, med_cubing, best_cubing,cubing4, storage)
            ################计算订单信息中SKU的纯放和混放比#################
        ...
        width = 0.8
        # 绘制柱状图, 每根柱子的颜色为紫罗兰色
        p2 = plt.bar(index, values, width, label="percentage of single&mix",color = 'r')
        # 设置横轴标签
